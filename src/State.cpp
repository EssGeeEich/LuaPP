...
State::State() : m_state(luaL_newstate()) {}
State::State(State&& o) : m_state(nullptr) { *this = std::move(o); }
...
void State::setSelf(std::weak_ptr<State> self) { m_self = self; }
void State::close() {
	if(!m_state)
		return;
	lua_close(m_state);
...
lua_State* State::GetState() const noexcept { return m_state; }
